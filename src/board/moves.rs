use std::fmt;

use super::piece::Piece;
use super::square::Square;

/// Move flags — classic CPW 4-bit encoding.
///
/// bits 0-5:  from square
/// bits 6-11: to square
/// bits 12-15: flag (see constants below)
#[derive(Copy, Clone, PartialEq, Eq, Default, Hash, Debug)]
pub struct Move(pub u16);

// Move flag constants (upper 4 bits of a Move)
pub const QUIET: u16 = 0;
pub const DOUBLE_PUSH: u16 = 1;
pub const CASTLE_KINGSIDE: u16 = 2;
pub const CASTLE_QUEENSIDE: u16 = 3;
pub const CAPTURE: u16 = 4;
pub const EN_PASSANT: u16 = 5;
// 6, 7 unused
pub const PROMO_KNIGHT: u16 = 8;
pub const PROMO_BISHOP: u16 = 9;
pub const PROMO_ROOK: u16 = 10;
pub const PROMO_QUEEN: u16 = 11;
pub const PROMO_CAPTURE_KNIGHT: u16 = 12;
pub const PROMO_CAPTURE_BISHOP: u16 = 13;
pub const PROMO_CAPTURE_ROOK: u16 = 14;
pub const PROMO_CAPTURE_QUEEN: u16 = 15;

impl Move {
    pub const NULL: Self = Self(0);

    #[inline(always)]
    pub fn new(from: Square, to: Square, flags: u16) -> Self {
        Self(from.0 as u16 | ((to.0 as u16) << 6) | (flags << 12))
    }

    #[inline(always)]
    pub fn from_sq(self) -> Square {
        Square((self.0 & 0x3F) as u8)
    }

    #[inline(always)]
    pub fn to_sq(self) -> Square {
        Square(((self.0 >> 6) & 0x3F) as u8)
    }

    #[inline(always)]
    pub fn flags(self) -> u16 {
        self.0 >> 12
    }

    #[inline(always)]
    pub fn is_capture(self) -> bool {
        // flags 4,5,12-15 are captures
        let f = self.flags();
        f == CAPTURE || f == EN_PASSANT || f >= PROMO_CAPTURE_KNIGHT
    }

    #[inline(always)]
    pub fn is_promo(self) -> bool {
        self.flags() >= PROMO_KNIGHT
    }

    #[inline(always)]
    pub fn is_castling(self) -> bool {
        let f = self.flags();
        f == CASTLE_KINGSIDE || f == CASTLE_QUEENSIDE
    }

    #[inline(always)]
    pub fn is_en_passant(self) -> bool {
        self.flags() == EN_PASSANT
    }

    #[inline(always)]
    pub fn is_quiet(self) -> bool {
        self.flags() == QUIET
    }

    #[inline(always)]
    pub fn is_double_push(self) -> bool {
        self.flags() == DOUBLE_PUSH
    }

    /// Promotion piece (only valid when `is_promo()` is true).
    #[inline(always)]
    pub fn promo_piece(self) -> Piece {
        match self.flags() & 3 {
            0 => Piece::Knight,
            1 => Piece::Bishop,
            2 => Piece::Rook,
            _ => Piece::Queen,
        }
    }

    /// Is this a null move sentinel?
    #[inline(always)]
    pub fn is_null(self) -> bool {
        self.0 == 0
    }
}

// -----------------------------------------------------------------------
// MoveList — stack-allocated fixed-capacity list
// -----------------------------------------------------------------------

/// Maximum legal moves in any chess position (theoretical maximum is 218).
pub const MAX_MOVES: usize = 256;

/// A stack-allocated list of up to [`MAX_MOVES`] moves.
///
/// Avoids heap allocation in the hot path — move generation is called at
/// every node of the search tree, so eliminating `Vec` allocations is
/// critical for performance.  At 2 bytes per move and 256 slots, the list
/// occupies 512 bytes (8 cache lines) on the stack.
#[derive(Clone, Debug)]
pub struct MoveList {
    moves: [Move; MAX_MOVES],
    len: usize,
}

impl MoveList {
    /// Create an empty list.
    #[inline(always)]
    pub fn new() -> Self {
        Self {
            moves: [Move(0); MAX_MOVES],
            len: 0,
        }
    }

    /// Append a move.
    #[inline(always)]
    pub fn push(&mut self, mv: Move) {
        debug_assert!(
            self.len < MAX_MOVES,
            "MoveList overflow — more than {MAX_MOVES} moves"
        );
        // SAFETY: chess positions have at most 218 legal moves; MAX_MOVES = 256.
        unsafe {
            *self.moves.get_unchecked_mut(self.len) = mv;
        }
        self.len += 1;
    }

    #[inline(always)]
    pub fn len(&self) -> usize {
        self.len
    }

    #[inline(always)]
    pub fn is_empty(&self) -> bool {
        self.len == 0
    }

    /// View the populated portion as a slice.
    #[inline(always)]
    pub fn as_slice(&self) -> &[Move] {
        // SAFETY: len ≤ MAX_MOVES; all elements up to len are initialised.
        unsafe { self.moves.get_unchecked(..self.len) }
    }

    /// Mutable view of the populated portion (e.g. for in-place sorting).
    #[inline(always)]
    pub fn as_mut_slice(&mut self) -> &mut [Move] {
        unsafe { self.moves.get_unchecked_mut(..self.len) }
    }

    /// Iterate over move references.
    #[inline(always)]
    pub fn iter(&self) -> std::slice::Iter<'_, Move> {
        self.as_slice().iter()
    }
}

impl Default for MoveList {
    fn default() -> Self {
        Self::new()
    }
}

// ---- Consuming iterator ----

pub struct MoveListIntoIter {
    list: MoveList,
    index: usize,
}

impl Iterator for MoveListIntoIter {
    type Item = Move;
    #[inline(always)]
    fn next(&mut self) -> Option<Move> {
        if self.index < self.list.len {
            // SAFETY: index < len ≤ MAX_MOVES.
            let mv = unsafe { *self.list.moves.get_unchecked(self.index) };
            self.index += 1;
            Some(mv)
        } else {
            None
        }
    }
    #[inline(always)]
    fn size_hint(&self) -> (usize, Option<usize>) {
        let rem = self.list.len - self.index;
        (rem, Some(rem))
    }
}

impl ExactSizeIterator for MoveListIntoIter {}

impl IntoIterator for MoveList {
    type Item = Move;
    type IntoIter = MoveListIntoIter;
    #[inline(always)]
    fn into_iter(self) -> Self::IntoIter {
        MoveListIntoIter {
            list: self,
            index: 0,
        }
    }
}

// ---- Borrowing iterator ----

impl<'a> IntoIterator for &'a MoveList {
    type Item = &'a Move;
    type IntoIter = std::slice::Iter<'a, Move>;
    #[inline(always)]
    fn into_iter(self) -> Self::IntoIter {
        self.as_slice().iter()
    }
}

impl fmt::Display for Move {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.is_null() {
            return write!(f, "0000");
        }
        write!(f, "{}{}", self.from_sq(), self.to_sq())?;
        if self.is_promo() {
            write!(f, "{}", self.promo_piece().promo_char())?;
        }
        Ok(())
    }
}
